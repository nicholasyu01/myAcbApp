...
export function CalculatorForm({ onSubmit, isLoading }: CalculatorFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!ticker || !shares || !arrivalDate || isNaN(sharesNum) || sharesNum <= 0) {
      return;
    }
    ...
    <Card className="w-full max-w-lg shadow-lg">
      <CardHeader className="space-y-1">
        <div className="flex items-center gap-2">
          ...
          <CardTitle className="text-2xl">Canada Arrival Cost Basis Calculator</CardTitle>
        </div>
        <CardDescription>
          Calculate the deemed acquisition cost (step-up) for securities when becoming a Canadian tax resident
        </CardDescription>
      </CardHeader>
      <CardContent>
